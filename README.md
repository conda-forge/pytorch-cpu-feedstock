...
Once the `conda-forge` channel has been enabled, `libtorch, pytorch, pytorch-cpu` can be installed with `conda`:

```
conda install libtorch pytorch pytorch-cpu
```

or with `mamba`:

```
mamba install libtorch pytorch pytorch-cpu
```
